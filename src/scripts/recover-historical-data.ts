...
async function recover() {
  ...
  try {
    ...
    // 1. Fetch current PERPETUAL gainers
    ...
    const top10 = tickers
      .filter((t: any) => perpetuals.has(t.symbol) && t.symbol.endsWith('USDT') && parseFloat(t.quoteVolume) > 5000000 && !FILTER_KEYWORDS.some(k => t.symbol.includes(k)))
      .sort((a: any, b: any) => parseFloat(b.priceChangePercent) - parseFloat(a.priceChangePercent))
      .slice(0, 10);

    // 2. Define target marks (Today UTC 00:00 and 04:00)
    const today = new Date();
    today.setUTCHours(0, 0, 0, 0);
    const marks = [new Date(today)]; // 00:00 UTC (08:00 Beijing)
    
    const h4 = new Date(today);
    h4.setUTCHours(4, 0, 0, 0); // 04:00 UTC (12:00 Beijing)
    marks.push(h4);
    ...
    for (const mark of marks) {
      ...
      for (const r of records) {
        await sql.unsafe(
          'INSERT INTO top_gainers_logs (symbol, price_change_percent, last_price, observation_time, capture_time) VALUES ($1, $2, $3, $4, $5)',
          [r.symbol, r.price_change_percent, r.last_price, r.observation_time, r.capture_time]
        );
      }
